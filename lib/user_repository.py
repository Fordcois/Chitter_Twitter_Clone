...
class UserRepository:
    ...
    def Registration_Errors(self, email, username ):
        ...
        all_usernames = [user['username'].upper() for user in all_registered_users]
        all_emails = [user['email'].upper() for user in all_registered_users]
        
        if username.upper() in all_usernames:
            errors.append ('Username already taken - Please choose another name')
        if email.upper() in all_emails:
...
# Find a user by their Username
    def find(self, username):
        DictResponse = self._connection.execute('SELECT * from users WHERE username iLIKE %s', [username])
        if DictResponse ==[]:
            return None
        else:
    ...
    def create(self,name, email, username, password):
        ...
        print (f' New Account Created for {name} called {username}')
